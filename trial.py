...
stopLines = {'right': 590, 'left': 800, 'up': 535}
defaultStop = {'right': 580, 'left': 810, 'up': 545}
...
vehiclesTurned = {'right': {1:[], 2:[]}, 'left': {1:[], 2:[]}, 'up': {1:[], 2:[]}}
vehiclesNotTurned = {'right': {1:[], 2:[]}, 'left': {1:[], 2:[]}, 'up': {1:[], 2:[]}}
rotationAngle = 3
mid = {'right': {'x':705, 'y':445}, 'left': {'x':695, 'y':425}, 'up': {'x':695, 'y':400}}
...
class Previous:
    def prev(self,prevJn,current,emergency):
        ...
        if current==0:
            ...
            time=time_max-((weighted_avg/20 )*time_max)
            time=round(time)
        if time<=0:
            time=time_min
        return time
...
class Main:
    ...
    while True:
        ...
        count1Text = font.render("Emergency:" + str(emergency_up), True, white)
        screen.blit(background_surface, (961,733))
        screen.blit(count1Text, (961,733))
