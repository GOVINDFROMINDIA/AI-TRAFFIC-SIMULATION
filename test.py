...
allowedVehicleTypes = {'car': True, 'bus': True, 'truck': True, 'bike': True, 'ambulance':False}
...
"""text = font.render(prev, True, white, black)
...
def initialize(direction_number,emergency):
    minTime = randomGreenSignalTimerRange[0]
    maxTime = randomGreenSignalTimerRange[1]
    '''time_min=15 #minimum time for each timer
    time_max=20 #maximum alloted time for each
    if currentCount==0:
            defaultRed=time_max
    else:
            weighted_avg = round((0.8 * currentCount) + (0.2 * prevJn))
            defaultRed=time_max-((weighted_avg/20 )*time_max)
            defaultRed=round(defaultRed)
            if defaultRed<time_max:
                defaultRed=defaultRed-time_max
    if defaultRed<=0:
            defaultRed=time_min'''
    if direction_number==0 and emergency==True:
        ts1 = TrafficSignal(0, 0, random.randint(minTime,maxTime))
        signals.append(ts1)
        ts2 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts2)
        ...
        signals.append(ts4)
    elif direction_number==1 and emergency==True:
        ts1 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts1)
        ts2 = TrafficSignal(0, 0, random.randint(minTime,maxTime))
        signals.append(ts2)
        ts3 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts3)
        ts4 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts4)
    elif direction_number==2 and emergency==True:
        ts1 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts1)
        ts2 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts2)
        ts3 = TrafficSignal(0, 0, random.randint(minTime,maxTime))
        signals.append(ts3)
        ts4 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts4)
    elif direction_number==3 and emergency==True:
        ts1 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts1)
        ts2 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts2)
        ts3 = TrafficSignal(defaultRed, defaultYellow, random.randint(minTime,maxTime))
        signals.append(ts3)
        ts4 = TrafficSignal(0, 0, random.randint(minTime,maxTime))
        signals.append(ts4)
    if(randomGreenSignalTimer):
        ...
        signals.append(ts4)
    repeat(direction_number,emergency)

def repeat(direction_number,emergency):
    global currentGreen, currentYellow, nextGreen
    if emergency:
        signals[currentGreen].green = 0  # Set current green signal's green timer to 0
        signals[currentGreen].yellow = 0  # Set current green signal's yellow timer to 0
        signals[direction_number].red = defaultRed
    ...
    # reset all signal defaultRed of current signal to default/random defaultRed
    if(randomGreenSignalTimer):
...
"""def prev(direction_number,prevJn,current,emergency):
        time_min=15 #minimum time for each timer
        time_max=20 #maximum alloted time for each
        if emergency== True:
            signals[direction_number].red=0
            signals[direction_number].green=time_max
            signals[direction_number].yellow=0
        if current==0:
            defaultRed=time_max
        else:
            weighted_avg = round((0.8 * current) + (0.2 * prevJn))
            defaultRed=time_max-((weighted_avg/20 )*time_max)
            defaultRed=round(defaultRed)
            if defaultRed<time_max:
                defaultRed=defaultRed-time_max
...
def generateAmbulance():
    global emergency_up, emergency_down, emergency_right, emergency_left
    ...
    while(True):
        ...
        if ambulance.crossed==1:
            ...
            emergency_right=False
            emergency_up=False
def start_thread(direction_number,emergency):
    thread = threading.Thread(target=initialize, args=(direction_number,emergency))
    thread.start()
class Main:
    global allowedVehicleTypesList
    #prev_object=Previous()
    i = 0
    ...
    thread3.start()
    start_thread(2,emergency_left)
    start_thread(3,emergency_up)
    start_thread(0,emergency_right)
    start_thread(1,emergency_down)
    upCount=0
    downCount=0
    rightCount=0
    leftCount=0
    ...
    while True:
        ...
        for vehicle in simulation:  
            ...
            if vehicle.y>=542 and vehicle.direction=='up' :
                upCount+=1
                #if vehicle.type=='ambulance':
                    #emergency_up= True
            elif vehicle.y<542 and vehicle.direction=='up':
                upCount=0
            elif vehicle.y<348 and vehicle.direction=='down':
                downCount+=1
                #if vehicleType=='ambulance':
                    #emergency_direction='Up'
            elif vehicle.y>348 and vehicle.direction=='down':
                downCount=0
        ...
        count1Text = font.render("Up count: " + str(upCount), True, white)
        screen.blit(background_surface, (391, 550))
        screen.blit(count1Text, (391, 550))
        ...
        '''initialize(2,prevJnCount,leftCount,)
        initialize(3,prevJnCount4,upCount,)
        initialize(0,prevJnCount3,rightCount,)
        initialize(1,prevJnCount2,downCount,emergency_down)'''
